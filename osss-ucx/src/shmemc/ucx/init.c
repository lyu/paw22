...
register_symmetric_heap(size_t heapno, mem_info_t *mip)
{
    ...
    switch (proc.heaps.type[heapno]) {
        ...
        case SYMM_TYPE_LIBOMP:
            ...
            mp.address = omp_target_alloc(mp.length, hn - 1);
            break;
        default:
            shmemu_assert(0, MODULE ": unknown symmetric type %d", proc.heaps.type[heapno]);
            break;
    }
    ...
    switch (proc.heaps.type[heapno]) {
        case SYMM_TYPE_MAIN:
            ...
            break;
        case SYMM_TYPE_LIBOMP:
            /* TODO: heapno - 1 isn't actually correct */
            shmema_buddy_init((void*)mip->base, mip->len, heapno - 1);
            break;
        default:
            break;
    }
...
deregister_symmetric_heap(mem_info_t *mip)
{
    ...
    if (mip->type == SYMM_TYPE_LIBOMP) {
        /* TODO: omp_target_free; is the base correct? */
        omp_target_free((void*)mip->base, mip->id - 1);
    }
}
