...
void *
shmem_partition_malloc(size_t size, int partition_id)
{
    void *addr;

    if (shmemu_unlikely(size == 0)) {
        return NULL;
    }

    SHMEMT_MUTEX_PROTECT(addr = shmema_buddy_malloc(size, partition_id));

    shmem_barrier_all();

    SHMEMU_CHECK_ALLOC(addr, size);

    return addr;
}

void
shmem_partition_free(void *ptr, int partition_id)
{
    shmem_barrier_all();

    SHMEMT_MUTEX_PROTECT(shmema_buddy_free(ptr, partition_id));

    logger(LOG_MEMORY, "%s(addr=%p)", __func__, ptr);
}
