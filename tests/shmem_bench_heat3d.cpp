...
void send_facet(const facet_t FT, params_t& pr)
{
    ...
    #pragma omp target update from(fi.sbf[0:fi.bf_len]) if(pr.on_device)
    shmem_putmem_nbi(nbr_fi.rbf, fi.sbf, fi.bf_len * sizeof(real_t), fi.nbr_pe);
}
...
int main(int argc, char** argv)
{
                            ...
                            map(to:pr.rbfs[5][0:(pr.npt_x*pr.npt_y)])
    {
        ...
        for (size_t i = 0; i < pr.max_iter; i++) {
            ...
            for (int f = 0; f < int(facet_t::LAST); f++) {
                facet_info& fi = pr.fis[f];
                #pragma omp target update to(fi.rbf[0:fi.bf_len])
            }
