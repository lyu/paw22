...
#define IDX(i,j,k) ((i) * ((pr.npt_y + 2) * (pr.npt_z + 2)) + (j) * (pr.npt_z + 2) + (k))
...
// TODO
real_t *g_sd_flat_1, *g_sd_flat_2;
...
struct params_t {
    ...
    bool on_device;
    MPI_Request sreqs[int(facet_t::LAST)];
    MPI_Request rreqs[int(facet_t::LAST)];
};
...
void alloc_storage(params_t& pr)
{
    ...
    g_sd_flat_1 = pr.sd_flat_1;
    g_sd_flat_2 = pr.sd_flat_2;
    ...
    pr.rbfs[int(facet_t::XU)] = new real_t[pr.npt_y * pr.npt_z];
    pr.rbfs[int(facet_t::XD)] = new real_t[pr.npt_y * pr.npt_z];
    pr.rbfs[int(facet_t::YU)] = new real_t[pr.npt_x * pr.npt_z];
    pr.rbfs[int(facet_t::YD)] = new real_t[pr.npt_x * pr.npt_z];
    pr.rbfs[int(facet_t::ZU)] = new real_t[pr.npt_x * pr.npt_y];
    pr.rbfs[int(facet_t::ZD)] = new real_t[pr.npt_x * pr.npt_y];
}
...
void pack_send_buffer(const facet_t FT, params_t& pr)
{
    ...
    const real_t* const sdp = const_cast<const real_t*>(g_sd_flat_2);
    ...
    #pragma omp target teams distribute parallel for collapse(2) if(pr.on_device)
    for (size_t i = 0; i < x_r; i++) {
        for (size_t j = 0; j < y_r; j++) {
            for (size_t k = 0; k < z_r; k++) {
                sbf[i * y_r * z_r + j * z_r + k] = sdp[IDX(i + fi.isf_xs, j + fi.isf_ys, k + fi.isf_zs)];
            }
        }
    }
}
...
void send_facet(const facet_t FT, params_t& pr)
{
    facet_info& fi     = pr.fis[int(FT)];

    auto sbf = fi.sbf;
    #pragma omp target data use_device_ptr(sbf) if(pr.on_device)
    {
        #if USE_DOUBLE == 0
            MPI_Isend(sbf, fi.bf_len, MPI_FLOAT, fi.nbr_pe, int(reverse_facet_ud(FT)), comm_world, &pr.sreqs[int(FT)]);
        #else
            MPI_Isend(sbf, fi.bf_len, MPI_DOUBLE, fi.nbr_pe, int(reverse_facet_ud(FT)), comm_world, &pr.sreqs[int(FT)]);
        #endif
    }
}
        ...
        for (size_t j = 1; j <= pr.npt_y; j++) {
            ...
            for (size_t k = 1; k <= pr.npt_z; k++) {
                ...
                if (diff_x2 + diff_y2 + diff_z2 > ball_r2) {
                    g_sd_flat_2[IDX(i, j, k)] = ambient;
                } else {
                    g_sd_flat_2[IDX(i, j, k)] = hi_temp;
                }
            }
        }
    }
...
void unpack_recv_buffer_helper(const facet_t FT, params_t& pr)
{
    ...
    for (size_t i = 0; i < x_r; i++) {
        for (size_t j = 0; j < y_r; j++) {
            for (size_t k = 0; k < z_r; k++) {
                sdp[IDX(i + fi.osf_xs, j + fi.osf_ys, k + fi.osf_zs)] = rbf[i * y_r * z_r + j * z_r + k];
            }
        }
    }
}
...
{
    for (int i = 0; i < int(facet_t::LAST); i++) {
        facet_info& fi = pr.fis[i];
        auto rbf = fi.rbf;
        #pragma omp target data use_device_ptr(rbf) if(pr.on_device)
        {
            #if USE_DOUBLE == 0
                MPI_Irecv(rbf, fi.bf_len, MPI_FLOAT, fi.nbr_pe, i, comm_world, &pr.rreqs[i]);
            #else
                MPI_Irecv(rbf, fi.bf_len, MPI_DOUBLE, fi.nbr_pe, i, comm_world, &pr.rreqs[i]);
            #endif
        }
    }
    for (int i = 0; i < int(facet_t::LAST); i++) {
        MPI_Wait(&pr.rreqs[i], MPI_STATUS_IGNORE);
    }
    for (int i = 0; i < int(facet_t::LAST); i++) {
        MPI_Wait(&pr.sreqs[i], MPI_STATUS_IGNORE);
    }
...
void update_facet(const facet_t FT, params_t& pr)
{
    facet_info& fi = pr.fis[int(FT)];
    ...
    for (size_t i = fi.isd_xs; i <= fi.isd_xe; i++) {
        for (size_t j = fi.isd_ys; j <= fi.isd_ye; j++) {
            for (size_t k = fi.isd_zs; k <= fi.isd_ze; k++) {
                const real_t u = weight * ( g_sd_flat_1[IDX(i-1, j, k)] + g_sd_flat_1[IDX(i+1, j, k)]
                                          + g_sd_flat_1[IDX(i, j-1, k)] + g_sd_flat_1[IDX(i, j+1, k)]
                                          + g_sd_flat_1[IDX(i, j, k-1)] + g_sd_flat_1[IDX(i, j, k+1)]
                                          - 6.0 * g_sd_flat_1[IDX(i, j, k)]
                                          );

                g_sd_flat_2[IDX(i, j, k)] = g_sd_flat_1[IDX(i, j, k)] + u;
            }
        }
    }
}
...
void update_interior(params_t& pr)
{
    const real_t weight = pr.K * pr.dt / (pr.ds * pr.ds);
...
int main(int argc, char** argv)
{
    ...
    std::swap(g_sd_flat_1, g_sd_flat_2);

    size_t iter = 0;

    if (pr.mype == 0) {
        std::cout << "3D heat transfer benchmark: sub-domain mesh "
    ...
    #pragma omp target data map(to:pr) \
                            map(tofrom:pr.sd_flat_1[0:((pr.npt_x+2)*(pr.npt_y+2)*(pr.npt_z+2))]) \
                            map(tofrom:pr.sd_flat_2[0:((pr.npt_x+2)*(pr.npt_y+2)*(pr.npt_z+2))]) \
                            map(to:pr.sbfs[0][0:(pr.npt_y*pr.npt_z)]) \
                            map(to:pr.sbfs[1][0:(pr.npt_y*pr.npt_z)]) \
                            map(to:pr.sbfs[2][0:(pr.npt_x*pr.npt_z)]) \
                            map(to:pr.sbfs[3][0:(pr.npt_x*pr.npt_z)]) \
                            map(to:pr.sbfs[4][0:(pr.npt_x*pr.npt_y)]) \
                            map(to:pr.sbfs[5][0:(pr.npt_x*pr.npt_y)]) \
                            map(to:pr.rbfs[0][0:(pr.npt_y*pr.npt_z)]) \
                            map(to:pr.rbfs[1][0:(pr.npt_y*pr.npt_z)]) \
                            map(to:pr.rbfs[2][0:(pr.npt_x*pr.npt_z)]) \
                            map(to:pr.rbfs[3][0:(pr.npt_x*pr.npt_z)]) \
                            map(to:pr.rbfs[4][0:(pr.npt_x*pr.npt_y)]) \
                            map(to:pr.rbfs[5][0:(pr.npt_x*pr.npt_y)])
    {
        timespec t0, t1;
        MPI_Barrier(comm_world);
        ...
        for (size_t i = 0; i < pr.max_iter; i++) {
            // Update the six facets and initialize their send buffers
            for (int f = 0; f < int(facet_t::LAST); f++) {
                update_facet(facet_t(f), pr);
                pack_send_buffer(facet_t(f), pr);
            }

            // Each thread send the facet(s) that it is responsible for
            for (int f = 0; f < int(facet_t::LAST); f++) {
                send_facet(facet_t(f), pr);
            }

            // Update the interior
            update_interior(pr);

            // Record the number of finished iterations
            iter = i + 1;
        ...
        if (pr.mype == 0) {
            std::cout << "Time elapsed: " << timediff_us(t0, t1) << " us"
                      << "\nIterations: " << iter << '\n';
        }
    }
